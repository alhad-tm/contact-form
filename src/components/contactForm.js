...
//  <span className="text-white text-xl lg:text-2xl xl:text-3xl uppercase">Contact </span>
...
//       <button className="sub-btn" type="submit" disabled={loading}>{loading ? 'Sending...' : 'Submit'}</button>
...
const ContactForm = () => {
  ...
  return (
    <div className="flex flex-col gap-2 items-center justify-center w-full">
      <span className="text-gray-400 text-xl lg:text-2xl xl:text-3xl uppercase">Contact </span>
      <form className="form" onSubmit={handleSubmit}>
        <input className="input-box" type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Name" />
        <input className="input-box" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Email" />
        <textarea className="input-box" name="message" value={formData.message} onChange={handleChange} placeholder="Message"></textarea>
        <button className="sub-btn" type="submit" disabled={loading}>{loading ? 'Sending...' : 'Submit'}</button>
        {errorMessage && <p className="text-sm text-gray-500">{errorMessage}</p>}
        {showSuccessMessage && <p className="text-green-500 text-base">Message sent successfully!</p>}
      </form>
      <motion.span
       initial={{ opacity: 0, y: -20 }}
       whileInView={{ opacity: 1, y: 0 }}
       transition={{ duration: 0.7,delay:0.2}} 
        className="text-gray-500 text-sm px-2 text-center">Any other queries contact @ <a className="text-white" href="mailto:your.email@example.com">email@example.com</a>
        </motion.span>
